...
const char* FL11_doc = R"(FL11)";
const char* BLK17_doc = R"(BLK17)";

} // namespace

py::array_t<double> BLK17_sensitivity(const py::array_t<double>& points_,
...
py::array_t<double> FL11_sensitivity(const py::array_t<double>& points_,
    const py::array_t<double>& weights_,
    const py::array_t<double>& centers_,
    const double alpha) {
    if (points_.shape(1) != centers_.shape(1)) {
...
PYBIND11_MODULE(sensitivity, m) {
    ...
    m.def("FL11_sensitivity", FL11_sensitivity, FL11_doc);
    m.def("BLK17_sensitivity", BLK17_sensitivity, BLK17_doc);
}
