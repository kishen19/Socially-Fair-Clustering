...
def process(args,q):
    algo,k,J,cor_num,init_num,iter,data,groups,coreset,d,ell,z,centers,n_samples,sample_size = args
    try:
        if algo == "ALGO":
            if J==0:
            ...
            if J==0:
                ...
                if iter==1:
                    new_centers, time_taken = run_algo_proj2(data,groups,k,d,ell,z,J,n_samples=n_samples,sample_size=sample_size,init_partition=centers)
                else:
                    new_centers, time_taken = run_algo_proj2(data,groups,k,d,ell,z,J,n_samples=n_samples,sample_size=sample_size,centers=centers)
        elif algo == "ALGO3":
            if J==0:
                new_centers, time_taken = run_algo(data,groups,k,d,ell,z,centers=centers)
            else:
                if iter==1:
                    new_centers, time_taken = run_algo_proj(data,k,d,ell,z,J,init_partition=centers)
                else:
                    new_centers, time_taken = run_algo_proj(data,k,d,ell,z,J,centers=centers)
        elif algo=="Lloyd":
            ...
            if iter==1:
                new_centers, time_taken = run_PCA(data,k,d,ell,z,J,init_partition=centers)
            else:
                new_centers, time_taken = run_PCA(data,k,d,ell,z,J,centers=centers)
                        ...
                        for cor_num in results.result[algo][k][J]:
                            ...
                            for init_num in results.result[algo][k][J][cor_num]:
                                if results.result[algo][k][J][cor_num][init_num]["num_iters"] == iter-1:
                                    ...
                                    job = pool.apply_async(process,([algo,k,J,cor_num,init_num,iter,data,results.groups,coreset,d,ell,Z,centers,ALGO2_N_SAMPLES,ALGO2_SAMPLE_SIZE],q))
                                    jobs.append(job)
